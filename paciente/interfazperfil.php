...
<html>
    ...
    <!-- La principal función de esta interfaz es generar una tabla con todos los datos introducidos por el mismo paciente en el registro -->
    ...
    <body>
        ...
        $consulta = $conectarBD->prepare("SELECT * FROM usuario WHERE usuario.Dni = '$DNIUsuario'");
        $consulta2 = $conectarBD->prepare("SELECT * FROM tablaunion WHERE tablaunion.Dniu = '$DNIUsuario'");
        ?>
        <div class="barra">
            <ul>
        ...
        <table>
            ...
            if(isset($usuario2['Dnif'])){
                $DNIF= $usuario2['Dnif'];

            $consulta3 = $conectarBD->prepare("SELECT * FROM facultativo WHERE facultativo.Dni ='$DNIF' ");
            ...
            if(isset($usuario2['Dnif'])){
            echo "<tr><th>Médico</th><td>" . $usuario3['Nombre'] . " " . $usuario3['Apellidos'] . "</td></tr>";
            }
            ?>
